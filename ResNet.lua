...
local function createModel(classes, conf)
  ...
  if pretrain ~= nil then
    ...
    --main:add(xconv(1024, 512, 3, 3, 1, 1, 1, 1, 'N', 0, 0, true))

    branch:add(nn.ConcatTable()
    :add(nn.Sequential():add(xconv(2048, 5*conf.bpc[1], 3, 3, 1, 1, 1, 1, 'N', 0, 0, false))
    :add(nn.Transpose({2,3},{3,4}))
    :add(nn.Reshape(-1, 5)))
    :add(nn.Sequential():add(xconv(2048, conf.classes*conf.bpc[1], 3, 3, 1, 1, 1, 1, 'N', 0, 0, false))
    :add(nn.Transpose({2,3},{3,4}))
    :add(nn.Reshape(-1, 2))))
