...
function GetPriorBBoxes(cfg)
   ...
   a_centers:mul(ang):add(-90+ang/2)   
...
function MatchingBBoxes(prior_bboxes, gt_bboxes, cfg)                       --adapted to grasp
  CorrAng = {{1,6},{1,2},{1,2},{2,3},{2,3},{3,4},{3,4},{4,5},{4,5},{5,6},{5,6},{1,6}}
  local msize = cfg.msize[1]
  local bpc = cfg.bpc[1]
  ...
  for i = 1, gt_bboxes:size(1) do
      ...
      elseif gt_bboxes[i][1] >= 320 then
            ...
            W = math.ceil(gt_bboxes[i][1]/(320/msize))
      end
      ...
      elseif gt_bboxes[i][2] >= 320 then
            ...
            H = math.ceil(gt_bboxes[i][2]/(320/msize))
      end

      local A1,A2
      local A = math.ceil( (gt_bboxes[i][5]+90)/(90/bpc) )
      if A==0 then 
            A=1
      end
      A1 = CorrAng[A][1]
      ...
      --print(H,W,A)
      local BT = torch.zeros(msize, msize, bpc)
      if H == H and W == W and A1 == A1 and A2==A2 then  
        BT[H][W][A1] = 1
        BT[H][W][A2] = 1
        local idx =torch.gt(BT,0)
        idx:view(prior_bboxes:size(1))
        match_indices[idx] = 2                      -- assign location value to 
...
function MultiBoxLoss(loc_preds, conf_preds, gt_bboxes, cfg)
  local prior_bboxes = GetPriorBBoxes(cfg)                                                    -- produce anchor boxes
  local match_indices, gt_locs = MatchingBBoxes(prior_bboxes, gt_bboxes, cfg)                 -- match anchor boxes with ground-truth
...
function Top1Detect(model, imgs, cfg)
  ...
  for i = 1, imgs:size(1) do
    ...
    local idx = torch.squeeze(softmax_conf[{{},{2}}]:gt(0.2),2):nonzero()       --
...
function PaddingBBox(img, bbox)
        --print(bbox[1])
        m = img:size()
        local x1 = math.ceil(bbox[1])-480
	local y1 = math.ceil(bbox[2])-480
        local x2 = math.ceil(bbox[3])+480
	local y2 = math.ceil(bbox[4])+480
        --print(x1, y1, x2, y2)
        ...
        if m>n then
        	paddedImage = torch.Tensor(3,m,m):fill(0)
                local startp, endp = math.ceil(m/2-n/2), math.floor(m/2-n/2)+n
                paddedImage[{{},{startp, endp },{}}] = Cropped		
	else
        	paddedImage = torch.Tensor(3,n,n):fill(0)
                local startp, endp = math.ceil(n/2-m/2), math.floor(n/2-m/2)+m
                paddedImage[{{},{},{startp, endp }}] = Cropped				
	end
	--return Cropped
        return paddedImage
