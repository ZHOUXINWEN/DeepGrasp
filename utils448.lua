...
function pointCrop(prior_bboxes,Xran,Yran)   --utils
  ...
  local row_mask = torch.gt(torch.lt(Croped,0)+torch.gt(Croped,448),0):sum(2)    -- the rows  zero remain byteTensor
...
function ImageCrop(img,Xran,Yran)
  local XOriN=90+Xran
  local YOriN=20+Yran
  local ImageCroped=image.crop(img,XOriN,YOriN,XOriN+448,YOriN+448) --the box of crop should be lower than 350 pixels
  return ImageCroped
end
...
function MatchingBBoxes(prior_bboxes, gt_bboxes)                       --adapted to grasp
  ...
  for i = 1, gt_bboxes:size(1) do
      local W = math.ceil(gt_bboxes[i][1]/32)
      local H = math.ceil(gt_bboxes[i][2]/32)
      local A
...
function MineHardExamples(conf_preds, match_indices)     -- adapted to grasp
  ...
  local num_sel = math.min(num_matches * 2, match_indices:eq(0):sum())   -- set the num of selected
  -- calc loss
...
function evaluate(model, testpath, img_dir, epoch)
	model:evaluate()
	local per_img_acc = {}
	local pred_num = 0
	local succ_pred = 0
